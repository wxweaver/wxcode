...
public:
    ...
    {
        ObjectToXrcFilter xrc(obj, "wxPlotWindow",
                              obj->GetPropertyAsString("name"));
        xrc.AddWindowProperties();
        xrc.AddProperty(_("style"), "style", XRC_TYPE_BITLIST);
        return xrc.GetXrcObject();
    }

    TiXmlElement* ImportFromXrc(TiXmlElement* xrcObj)
    {
        XrcToXfbFilter filter(xrcObj, "wxPlotWindow");
        filter.AddWindowProperties();
        filter.AddProperty(_("style"), "style", XRC_TYPE_BITLIST);
...
public:
    wxObject* Create(IObject* obj, wxObject* parent)
    {
        ...
        led->SetState((awxLedState)obj->GetPropertyAsInteger("state"));
        return led;
    }
};
...
public:
    wxObject* Create(IObject* obj, wxObject* parent)
    {
        ...
        ledbg->SetDrawingMode((wxLedBarGraphDrawMode)obj->GetPropertyAsInteger("drawing_mode"));
        ledbg->SetSizingMode((wxLedBarGraphSizeMode)obj->GetPropertyAsInteger("sizing_mode"));
        ledbg->SetOrientation((wxOrientation)obj->GetPropertyAsInteger("orientation"));

        ledbg->SetBarWidths(obj->GetPropertyAsInteger("bar_widths"));
        ledbg->SetBarCount(obj->GetPropertyAsInteger("bar_count"));

        double max = obj->GetPropertyAsFloat("max_value");
        double min = obj->GetPropertyAsFloat("min_value");

        ledbg->SetMaxValue(max);
        ledbg->SetMinValue(min);

        double val = obj->GetPropertyAsFloat("value");
        val = val * (max - min) + min;
        ledbg->SetValue(val);
...
MACRO(wxLED_BG_DOUBLE_SIDED)
MACRO(wxLED_BG_SINGLE_SIDED_TOP_LEFT)
MACRO(wxLED_BG_SINGLE_SIDED_BOTTOM_RIGHT)
MACRO(wxLED_BG_FIXED_BAR_COUNT)
MACRO(wxLED_BG_FIXED_BAR_SIZE)
